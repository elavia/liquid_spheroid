	:: Bash script for calculation of S and R in one point. The inputs are 'c', 'm', 'n', 'eta', 'xi' 
	:: Sintaxis:
	...
	obl_sphwv -max_memory %max_mem% -precision %prec% -verbose n -c %1 -m %2 -n %3 -w dr_neg -n_dr_neg %n_dr%_neg -dr_neg_min %dr_neg_min%
	...
	obl_sphwv -max_memory %max_mem% -precision %prec% -verbose n -c %1 -m %2 -n %3 -w S1 -a %4 -b %4 -d 0.01 -arg_type eta -p 34>Out_S.dat
	:: Spheroidal wave function R
	obl_sphwv -max_memory %max_mem% -precision %prec% -verbose n -c %1 -m %2 -n %3 -w R -a %5 -b %5 -d 0.1 	-arg_type xi -which R1_1,R1_2,R2_1,R2_2 -p 34>Out_R.dat
