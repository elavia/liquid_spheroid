	::  Bahs script for calculation of S (grid) and S incident. The inputs are 'c','m','n','eta_inc','grid_step'
	::  Sintaxis:
	...
	pro_sphwv -max_memory %max_mem% -precision %prec% -verbose n -c %1 -m %2 -n %3 -w S1 -a %4 -b %4 -d 0.01 -arg_type eta -p 34 > Out_Sinc.dat 
	::  Spheroidal wave function S
 	pro_sphwv -max_memory %max_mem% -precision %prec% -verbose n -c %1 -m %2 -n %3 -w S1 -a -1.0 -b 1.0000001 -d %5 -arg_type eta -p 34 > Out_S.dat
